...
class DeploymentsManager {
    constructor(env) {
        ...
        log('adding helpers');
        this.deploymentsExtension = helpers_1.addHelpers(env, this, partialExtension, partialExtension.getArtifact, async (name, deployment, artifactName) => {
            if (artifactName &&
    ...
    async setupAccounts() {
        if (!this.db.accountsLoaded) {
            ...
            const { namedAccounts, unnamedAccounts, unknownAccounts, addressesToProtocol, } = utils_1.processNamedAccounts(this.env, accounts, chainId);
            if (this.env.network.name === 'hardhat' &&
