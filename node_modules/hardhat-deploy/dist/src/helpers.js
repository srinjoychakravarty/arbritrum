...
function addHelpers(env, deploymentManager, partialExtension, // TODO
getArtifact, saveDeployment, willSaveToDisk, onPendingTx, getGasPrice, log, print) {
    async function init() {
        if (!provider) {
            provider = new providers_1.Web3Provider(fixProvider(env.network.provider));
            try {
        ...
        if (typeof options.proxy === 'object') {
            ...
            if (options.proxy.proxyContract) {
                if (typeof options.proxy.proxyContract === 'string') {
                    try {
                        proxyContract = await env.deployments.getExtendedArtifact(options.proxy.proxyContract);
                    }
                    catch (e) { }
...
`);
        ...
        if (viaAdminContract) {
            ...
            if (typeof proxyAdminArtifactNameOrContract === 'string') {
                try {
                    proxyAdminContract = await env.deployments.getExtendedArtifact(proxyAdminArtifactNameOrContract);
                }
                catch (e) { }
            ...
            if (!proxy) {
                ...
                if (proxyAdminName) {
                    ...
                    if (changeImplementationMethod === 'upgradeToAndCall' &&
                        !updateMethod) {
                        executeReceipt = await execute(proxyName, Object.assign(Object.assign({}, options), { from: currentOwner }), 'upgrade', implementation.address);
                    }
                    else {
    ...
    function _getFrom(from, optional) {
        ...
        if (from.length >= 64) {
            ...
            if (availableAccounts[from.toLowerCase()]) {
                ...
                if (deploymentManager.addressesToProtocol[from.toLowerCase()] === 'ledger')
                    ethersSigner = new hardware_wallets_1.LedgerSigner(provider);
